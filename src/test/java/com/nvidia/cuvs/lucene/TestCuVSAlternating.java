...
public class TestCuVSAlternating extends LuceneTestCase {
  ...
  protected static Logger log = Logger.getLogger(TestCuVSAlternating.class.getName());
  ...
  static int randomDataSetSize;
  static int randomDataSetDimension;
  static int randomTopK;
  ...
  public static void beforeClass() throws Exception {
    ...
    randomDataSetSize = random.nextInt(1000, 10_000);
    randomDataSetDimension = 1024;
    randomTopK = random.nextInt(1, 32);
    log.info(
        "Dataset size: "
            + randomDataSetSize
            + ", Dimension: "
            + randomDataSetDimension
            + ", TopK: "
            + randomTopK);
    dataset = generateDataset(random, randomDataSetSize, randomDataSetDimension);
  ...
  public void testVectorSearchWithAlternatingDocuments() throws IOException {
    ...
    Query query = new KnnFloatVectorQuery("vector", queryVector, randomTopK);
    ScoreDoc[] hits = searcher.search(query, randomTopK).scoreDocs;

    // Verify we get exactly TOP_K results
    assertEquals("Should return exactly " + randomTopK + " results", randomTopK, hits.length);
    ...
    List<Integer> expectedIds = calculateExpectedTopK(queryVector, randomTopK);
  ...
  public void testVectorSearchWithFilterAndAlternatingDocuments() throws IOException {
    ...
    Query filteredQuery =
        new CuVSKnnFloatVectorQuery("vector", queryVector, randomTopK, filter, randomTopK, 1);
    ScoreDoc[] filteredHits = searcher.search(filteredQuery, randomTopK).scoreDocs;
  ...
  private List<Integer> calculateExpectedTopK(float[] query, int topK) {
    ...
    for (int i = 0; i < randomDataSetSize; i += 2) {
      double distance = 0;
      for (int j = 0; j < randomDataSetDimension; j++) {
